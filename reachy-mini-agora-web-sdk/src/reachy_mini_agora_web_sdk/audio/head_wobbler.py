...
from reachy_mini_agora_web_sdk.audio.speech_tapper import HOP_MS, SwayRollRT


SAMPLE_RATE = 24000
MOVEMENT_LATENCY_S = 0.06  # seconds between audio and robot movement
...
class HeadWobbler:
    ...
    def feed(self, delta_b64: str) -> None:
        ...
        self.feed_with_sample_rate(delta_b64, SAMPLE_RATE)

    def feed_with_sample_rate(self, delta_b64: str, sample_rate: int) -> None:
        """Thread-safe: push audio into the consumer queue with explicit sample rate."""
        buf = np.frombuffer(base64.b64decode(delta_b64), dtype=np.int16).reshape(1, -1)
        sr = int(sample_rate) if sample_rate else SAMPLE_RATE
